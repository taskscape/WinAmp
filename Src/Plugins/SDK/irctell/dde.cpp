...
// DWORD_PTR (not DWORD) for the last two parameters: PFNCALLBACK declares them
// pointer-sized, which differs from DWORD on 64-bit builds.
HDDEDATA CALLBACK DdeGenericCallback(UINT uType, UINT uFmt,HCONV hconv, HSZ hsz1,
                                     HSZ hsz2, HDDEDATA hdata, DWORD_PTR dwData1,
                                     DWORD_PTR dwData2)
{
	return ((HDDEDATA)0);
}
