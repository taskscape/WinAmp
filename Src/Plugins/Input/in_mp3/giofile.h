...
// ../vlb/dataio.h is no longer included: VLB playback was removed because the
// Dolby VLB decoder sources are not redistributable, so CGioFile no longer
// needs to satisfy the DataIOControl interface.
#include "ID3v2.h"
#include "LAMEInfo.h"
...
public:
	...
	// serves the MPEGSTREAM_EOF entry of the ifc_mpeg_stream_reader dispatch
	// table; previously it doubled as the DataIOControl::EndOf override, which
	// was dropped together with VLB playback (Dolby redistribution rights).
	int EndOf(void)
	{
		return IsEof();
	}
