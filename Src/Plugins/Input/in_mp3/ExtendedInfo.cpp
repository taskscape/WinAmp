...
int winampGetExtendedFileInfoW(const wchar_t *fn, const char *data, wchar_t *dest, size_t destlen)
{
	...
	else if (!_stricmp(data, "family"))
	{
		...
		// VLB family mapping removed along with VLB playback (Dolby redistribution rights required)
		return 0;
	}
	else if (!_stricmp(data, "mime"))
	{
		...
		// audio/vlb MIME mapping removed along with VLB playback (Dolby redistribution rights required)
		return 0;
	}
