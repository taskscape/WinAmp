...
#include <shlwapi.h>
#include "adts.h"
// VLB playback removed: the Dolby VLB decoder sources are not redistributable
// without written permission from Winamp/Dolby, so no VLB fallback is built.
// adts_vlb.h used to pull in the Wasabi/Agave service headers transitively.
...
DWORD DecodeLoop::OpenDecoder()
{
	...
	if (isAac)
	{
		...
		// previously fell back to the ADTS_VLB decoder here; VLB support was
		// removed because its sources require redistribution rights from
		// Winamp/Dolby, so AAC now depends solely on the adts_aac service.
	}
	else
	{
...
// TODO?	if (decoder != &aacp) // hack because aac+ bitrate isn't accurate at this point
	...
	while (!killDecodeThread)
	{
		...
		if (need_prebuffer && file.IsStream() && maxlatency && !file.IsEof())
			PreBuffer();
